...
function scholar_scraper_init_styles() {
    if (
        wp_script_is( 'scholar_scraper_dashicons', 'registered' ) ||
        wp_script_is( 'scholar_scraper_dashicons', 'enqueued' ) ||
        wp_doing_ajax()
    ) {
        return;
    }
...
/**
 * Function that handles the actions to do when the admin is initialized.
 * @return void
 * @since 1.0.0
 */
...
function scholar_scraper_custom_block_script_register() {
    ...
    if ( wp_script_is( 'scholar_scraper_block_script', 'registered' )
         || wp_script_is( 'scholar_scraper_block_script', 'enqueued' )
         || wp_doing_ajax()
    ) {
        return;
    }
 ...
 * @throws ReflectionException Erreur survenue lors de l'instanciation d'un objet ReflectionClass.
 * @since 1.0.0
 */
function scholar_scraper_block_render_callback( array $attributes ): string {
    ...
    return scholar_scraper_display_result( $attributes );
}
